...
struct WasmWelcomeDialog : rack::widget::OpaqueWidget {
  ...
  WasmWelcomeDialog() {
    ...
    box.size = rack::math::Vec(750, 410);
    ...
    label->text =
        ""
        "Bem-vindo ao Cardinal na Web!\n"
        "\n"
        "Se estiver usando dispositivos móveis/touch, por favor, note:\n"
        " - Um toque rápido faz um clique simples do mouse\n"
        " - Pressionar e mover faz a ação de clicar e arrastar\n"
        " - Pressionar e segurar faz o clique direito (e abre o navegador de "
        "módulos)\n"
        "\n"
        "Ainda é um pouco experimental, caso encontre erros nos avise.\n"
        "Se ao áudio estiver estranho, clique em 'Engine' e aumente o valor de "
        "BufferSize.\n\n"
        "Divirta-se!";
    ...
    struct JoinDiscussionButton : Button {
      ...
      void onAction(const ActionEvent &e) override {
        patchUtils::openBrowser(
            "https://github.com/nucleomusicanova/Patches/issues/new");
    ...
    discussionButton->text = "Algum Erro?";
...
static void downloadRemotePatchFailed(const char *const filename) {
  ...
  CardinalPluginContext *const context =
      static_cast<CardinalPluginContext *>(APP);
  ...
  context->patch->templatePath =
      rack::system::join(asset::patchesPath(), "templates/main.vcv");
  context->patch->loadTemplate();
  context->scene->rackScroll->reset();
}

static void downloadRemotePatchSucceeded(const char *const filename) {
  d_stdout("downloadRemotePatchSucceeded %s | %s", filename,
           APP->patch->templatePath.c_str());
  CardinalPluginContext *const context =
      static_cast<CardinalPluginContext *>(APP);
  CardinalBaseUI *const ui = static_cast<CardinalBaseUI *>(context->ui);
  ...
  if (FILE *f = fopen(filename, "r")) {
    uint8_t buf[8] = {};
    fread(buf, 8, 1, f);
    d_stdout("read patch %x %x %x %x %x %x %x %x", buf[0], buf[1], buf[2],
  ...
  try {
    ...
    const std::string message =
        rack::string::f("Could not load patch: %s", e.what());
...
class CardinalUI : public CardinalBaseUI, public WindowParametersCallback {
#if !DISTRHO_PLUGIN_WANT_DIRECT_ACCESS
#ifdef DISTRHO_OS_WASM
  ...
  struct ScopedContext {
    ...
    ScopedContext(CardinalUI *const ui) : context(ui->context) {
      rack::contextSet(context);
      WindowParametersRestore(context->window);
    }

    ScopedContext(CardinalUI *const ui, const int mods) : context(ui->context) {
      rack::contextSet(context);
...
public:
  CardinalUI()
...
#if !DISTRHO_PLUGIN_WANT_DIRECT_ACCESS
#ifdef DISTRHO_OS_WASM
        fInitializer(new Initializer(
            static_cast<const CardinalBasePlugin *>(nullptr), this)),
#else
    ...
    context->patch->templatePath = context->patch->factoryTemplatePath =
        fInitializer->factoryTemplatePath;
    ...
    DISTRHO_SAFE_ASSERT(
        remoteUtils::connectToRemote(CARDINAL_DEFAULT_REMOTE_URL));
    ...
    if (rack::isStandalone() &&
        rack::system::exists(rack::settings::settingsPath)) {
      const double width =
          std::max(648.f, rack::settings::windowSize.x) * scaleFactor;
      const double height =
          std::max(538.f, rack::settings::windowSize.y) * scaleFactor;
    ...
    rack::widget::Widget *const browser =
        context->scene->browser->children.back();
    ...
    if (rack::asset::systemDir.empty()) {
      ...
      errorMessage = rack::string::f(
          "System directory \"%s\" does not exist. "
    ...
    if (rack::patchStorageSlug != nullptr) {
      ...
      static_cast<CardinalBasePlugin *>(context->plugin)
          ->setState("patch", rack::patchFromURL);
      rack::contextSet(context);
    } else {
  ...
  ~CardinalUI() override {
...
#if CARDINAL_VARIANT_MINI && !DISTRHO_PLUGIN_WANT_DIRECT_ACCESS
    {
      ...
      // do a little dance to prevent context scene deletion from saving to temp
      // dir
  ...
  void uiIdle() override {
    ...
    if (context->plugin->isSelfTestInstance()) {
      ...
      context->event->handleButton(mousePos, GLFW_MOUSE_BUTTON_LEFT,
                                   GLFW_RELEASE, 0x0);
      ...
      for (rack::plugin::Plugin *p : rack::plugin::plugins) {
        for (rack::plugin::Model *m : p->models) {
          d_stdout(">>>>>>>>>>>>>>>>> LOADING module %s : %s", p->slug.c_str(),
                   m->slug.c_str());
          rack::engine::Module *const module = m->createModule();
          DISTRHO_SAFE_ASSERT_CONTINUE(module != nullptr);

          rack::CardinalPluginModelHelper *const helper =
              dynamic_cast<rack::CardinalPluginModelHelper *>(m);
          DISTRHO_SAFE_ASSERT_CONTINUE(helper != nullptr);

          d_stdout(">>>>>>>>>>>>>>>>> LOADING moduleWidget %s : %s",
                   p->slug.c_str(), m->slug.c_str());
          rack::app::ModuleWidget *const moduleWidget =
              helper->createModuleWidget(module);
          DISTRHO_SAFE_ASSERT_CONTINUE(moduleWidget != nullptr);

          d_stdout(">>>>>>>>>>>>>>>>> ADDING TO ENGINE %s : %s",
                   p->slug.c_str(), m->slug.c_str());
          context->engine->addModule(module);

          d_stdout(">>>>>>>>>>>>>>>>> ADDING TO RACK VIEW %s : %s",
                   p->slug.c_str(), m->slug.c_str());
          ...
          d_stdout(">>>>>>>>>>>>>>>>> REMOVING FROM RACK VIEW %s : %s",
                   p->slug.c_str(), m->slug.c_str());
          context->scene->rack->removeModule(moduleWidget);
          app.idle();

          d_stdout(">>>>>>>>>>>>>>>>> DELETING module + moduleWidget %s : %s",
                   p->slug.c_str(), m->slug.c_str());
          delete moduleWidget;
    ...
    if (counterForFirstIdlePoint >= 0 && ++counterForFirstIdlePoint == 30) {
      ...
      if (rack::patchStorageSlug != nullptr) {
        ...
        emscripten_async_wget(url.c_str(), context->patch->templatePath.c_str(),
                              downloadRemotePatchSucceeded,
                              downloadRemotePatchFailed);
      } else if (rack::patchRemoteURL != nullptr) {
    ...
    if (windowParameters.rateLimit != 0 &&
        ++rateLimitStep % (windowParameters.rateLimit * 2))
      return;
  ...
  void WindowParametersChanged(const WindowParameterList param,
...
protected:
  ...
  void parameterChanged(const uint32_t index, const float value) override {
    ...
    if (index < kCardinalParameterCountAtMiniBuffers) {
      float *const buffer = *const_cast<float **>(
          &context->dataIns[index - kCardinalParameterStartMiniBuffers]);
      buffer[0] = value;
      return;
    }
    ...
    case kCardinalParameterMiniTimeFrame:
      context->frame =
          static_cast<uint64_t>(value * context->sampleRate + 0.5f);
      return;
    ...
    case kCardinalParameterMiniTimeBeatsPerMinute:
      ...
      context->ticksPerFrame =
          1.0 / (60.0 * context->sampleRate / context->beatsPerMinute /
                 context->ticksPerBeat);
      return;
  ...
  void stateChanged(const char *const key, const char *const value) override {
    ...
    if (std::strcmp(key, "patch") == 0) {
      ...
      FILE *const f = std::fopen(
          rack::system::join(fAutosavePath, "patch.json").c_str(), "w");
  ...
  static int glfwMods(const uint mod) noexcept {
    ...
    /*
    if (glfwGetKey(win, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS || glfwGetKey(win,
    GLFW_KEY_RIGHT_SHIFT) == GLFW_PRESS) mods |= GLFW_MOD_SHIFT; if
    (glfwGetKey(win, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS || glfwGetKey(win,
    GLFW_KEY_RIGHT_CONTROL) == GLFW_PRESS) mods |= GLFW_MOD_CONTROL; if
    (glfwGetKey(win, GLFW_KEY_LEFT_ALT) == GLFW_PRESS || glfwGetKey(win,
    GLFW_KEY_RIGHT_ALT) == GLFW_PRESS) mods |= GLFW_MOD_ALT; if (glfwGetKey(win,
    GLFW_KEY_LEFT_SUPER) == GLFW_PRESS || glfwGetKey(win, GLFW_KEY_RIGHT_SUPER)
    == GLFW_PRESS) mods |= GLFW_MOD_SUPER;
    */
  ...
  bool onMouse(const MouseEvent &ev) override {
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT &&
        (mods & RACK_MOD_MASK) == (GLFW_MOD_CONTROL | GLFW_MOD_SHIFT)) {
      button = GLFW_MOUSE_BUTTON_MIDDLE;
  ...
  bool onMotion(const MotionEvent &ev) override {
    ...
    const rack::math::Vec mousePos =
        rack::math::Vec(ev.pos.getX(), ev.pos.getY())
            .div(getScaleFactor())
            .round();
  ...
  bool onScroll(const ScrollEvent &ev) override {
    ...
    rack::math::Vec scrollDelta =
        rack::math::Vec(-ev.delta.getX(), ev.delta.getY());
  ...
  bool onKeyboard(const KeyboardEvent &ev) override {
    ...
    */
    ...
    const ScopedContext sc(this, mods);
    return context->event->handleKey(lastMousePos, key, ev.keycode, action,
                                     mods);
  }
  ...
  void onResize(const ResizeEvent &ev) override {
    ...
    if (context->window != nullptr)
      WindowSetInternalSize(
          context->window,
          rack::math::Vec(ev.size.getWidth(), ev.size.getHeight()));

    const double scaleFactor = getScaleFactor();
    const int width = static_cast<int>(ev.size.getWidth() / scaleFactor + 0.5);
    const int height =
        static_cast<int>(ev.size.getHeight() / scaleFactor + 0.5);
  ...
  void uiFileBrowserSelected(const char *const filename) override {
    ...
    if (saving) {
      ...
      try {
        ...
        std::string message =
            rack::string::f("Could not save patch: %s", e.what());
        asyncDialog::create(message.c_str());
        return;
      }
    } else {
...
UI *createUI() { return new CardinalUI(); }
